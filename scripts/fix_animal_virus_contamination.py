...
HUMAN_COLLECTIONS = {
    ...
    # Mouse Gut (8) is deliberately excluded: it is a mouse model, so murine and
    # rodent viruses are legitimate content, not contamination to strip.
    10: 'IBD Gut Virome',
    11: 'HIV+ Gut Virome',
...
def find_human_replacements(
    ...
    provided seeded RNG so replacement selection is reproducible.
    """
    ...
    """
    params = list(already_in_collection)
    candidates = [row[0] for row in conn.execute(query, params).fetchall()]
...
def fix_collection(
    ...
    for family, items in by_family.items():
        ...
        replacements = find_human_replacements(
            conn, family, len(items), remaining_ids, rng
        )
...
def main():
    ...
    for cid, cname in sorted(collections_to_check.items()):
        result = fix_collection(conn, cid, cname, rng, dry_run=args.dry_run)
