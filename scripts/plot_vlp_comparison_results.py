...
# Okabe-Ito colorblind-safe palette. All 8 hues plus a neutral gray for the
# trace low-complexity category. Adjacent stacked-bar categories are kept
# maximally distinct, and no red/green pairing is used.
COLORS = {
    'viral': '#009E73',                 # bluish green
    'dark_matter': '#E69F00',           # orange
    'bacterial_background': '#0072B2',  # blue
    'host_dna': '#D55E00',              # vermillion
    'rrna': '#56B4E9',                  # sky blue
    'fungal_background': '#CC79A7',     # reddish purple
    'phix': '#F0E442',                  # yellow
    'reagent_bacteria': '#000000',      # black
    'artifact_low_complexity': '#999999',  # neutral gray (trace category)
}
...
# Four-dataset categorical colors (Okabe-Ito subset, no red/green pairing).
DATASET_COLORS = ['#0072B2', '#E69F00', '#009E73', '#CC79A7']
...
def plot_composition():
    ...
    for cat in contam_cats:
        ...
        ax.bar(x, vals, 0.6, label=LABELS.get(cat, cat), color=COLORS.get(cat, '#999999'), bottom=bottom)
...
def plot_vlp_heatmap():
    ...
    # Add text. viridis is dark at low values, bright at high values, so use
    # white text on the dark (low) end and black text on the bright (high) end.
    for i in range(len(names)):
        for j in range(len(contam_cats)):
            ax.text(j, i, f'{data[i, j]:.0f}%', ha='center', va='center', fontsize=10,
                    color='black' if data[i, j] > 55 else 'white')
