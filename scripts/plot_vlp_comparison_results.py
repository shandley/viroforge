...
# Discover available datasets — prefer v2 (with bacterial/fungal background)
DATASET_CANDIDATES = [
    ('IBD 0.2μm', [
        'data/ibd_vlp_novaseq/metadata/ibd_gut_virome_inflammatory_bowel_disease_metadata.json',
    ]),
    ('IBD 0.45μm', [
        'data/ibd_vlp045_novaseq/metadata/ibd_gut_virome_inflammatory_bowel_disease_metadata.json',
    ]),
    ('Vag 0.2μm', [
        'data/vaginal_vlp_novaseq_v2/metadata/vaginal_virome_healthy_metadata.json',
        'data/vaginal_vlp_novaseq/metadata/vaginal_virome_healthy_metadata.json',
    ]),
    ('Vag 0.45μm', [
        'data/vaginal_vlp045_novaseq_v2/metadata/vaginal_virome_healthy_metadata.json',
        'data/vaginal_vlp045_novaseq/metadata/vaginal_virome_healthy_metadata.json',
    ]),
]

datasets = {}
for name, candidates in DATASET_CANDIDATES:
...
if not datasets:
    print("ERROR: No datasets found. Generate datasets first.")
    exit(1)
...
# Load virome-qc passports (optional — may not exist)
qc = {}
for name, meta_path in datasets.items():
    ds_dir = os.path.dirname(os.path.dirname(meta_path))
    passport_path = os.path.join(ds_dir, 'analysis', 'virome_qc', 'passport.json')
...
if __name__ == '__main__':
    ...
    if qc:
        ...
        print("Skipping virome-qc plots (no passport.json files found)")
