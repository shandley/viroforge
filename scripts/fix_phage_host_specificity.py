...
def find_replacements(conn, collection_id: int, count: int, exclude_ids: Set[str],
                      rng: random.Random) -> List[Dict]:
    """Find body-site-appropriate phage replacements.

    Candidates are fetched in a deterministic order and sampled with the
    provided seeded RNG, drawing from the primary source first, then secondary,
    then fallback, until `count` replacements are found. Reproducible for a
    given seed.
    """
    ...
    for source in ['primary', 'secondary', 'fallback']:
        if len(picked) >= count:
            break
        exclude_all = exclude_ids | picked_ids
        placeholders = ','.join('?' for _ in exclude_all) if exclude_all else "''"
        ...
        """
        candidates = [dict(row) for row in conn.execute(query, list(exclude_all)).fetchall()]
        need = count - len(picked)
        if len(candidates) > need:
            candidates = rng.sample(candidates, need)
        for c in candidates:
            picked.append(c)
            picked_ids.add(c['genome_id'])

    return picked[:count]


def fix_collection(conn, collection_id: int, collection_name: str, dry_run: bool,
    ...
    replacements = find_replacements(conn, collection_id, len(wrong), exclude_ids, rng)
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=42,
                        help='Seed for reproducible replacement selection (default: 42)')
    ...
    collections_to_fix = {
        ...
        # Mouse Gut (8) is excluded: it has no site-appropriate phage config, so
        # it would only remove global-match phages with no replacements.
        10: 'IBD Gut Virome',
        11: 'HIV+ Gut Virome',
        16: 'Vaginal Virome',
    }
    ...
    for cid, cname in sorted(collections_to_fix.items()):
        result = fix_collection(conn, cid, cname, args.dry_run, rng)
