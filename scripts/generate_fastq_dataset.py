    ...
    if is_long_read:
        ...
        if args.platform == 'pacbio-hifi':
            ...
            for i, (seq, abundance) in enumerate(zip(sequences, abundances)):
                ...
                # Guarantee at least 1 read: min depth = read_length / genome_length
                min_depth = platform_config.read_length_mean / genome_length
                genome_depth = max(genome_depth, min_depth)
                ...
                # Also check for un-numbered output
            ...
            logger.info(f"  PBSIM3 CLR generation complete: {len(per_genome_bams)} output files from {len(sequences)} genomes")
            ...
            logger.info(f"  PBSIM3 Nanopore generation complete: {len(all_nano_fastqs)} files from {len(sequences)} genomes")
